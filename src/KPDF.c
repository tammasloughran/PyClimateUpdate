...
typedef enum { updfepanechnikov, updfbiweight, updftriangular } UPDFMode;
...
/* Declae this function because it is accessed before it is defined */
...
static KernelFunctionPtr updfselectkf( UPDFMode mode, double *k )
{
  ...
  default:
    /*** Return null here ****/
    PyErr_SetString(PyExc_ValueError,"Only Epanechnikov, Biweight and Triangular kernels allowed");
    f=NULL;
    *k=1.;
    break;
  }
				...
				double bandwidth , UPDFMode mode )
{
  ...
  rarray=(PyArrayObject*)PyArray_FromDims(1,dimensions,PyArray_DOUBLE);
  if (rarray==NULL)
    return NULL;
  /** 
...
static PyObject *MPDFGrid2Array( PyArrayObject **numpys , int fastest )
{
  ...
  retarray=(PyArrayObject*)PyArray_FromDims(2,dimensions,PyArray_DOUBLE);
  if(retarray==NULL)
    return NULL;
